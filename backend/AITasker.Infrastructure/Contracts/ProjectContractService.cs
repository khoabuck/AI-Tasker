    ...
    public class ProjectContractService : IProjectContractService
    {
        ...
        public async Task<bool> CreateDraftContractAsync(CreateContractRequest request)
        {
            ...
            if (proposal.Status != "ACCEPTED")
                throw new InvalidOperationException("Only ACCEPTED proposals can create contracts.");

            var existingContract = await _context.ProjectContracts
                .AnyAsync(c => c.ProposalId == request.ProposalId);
            ...
            var feeAmount = request.FinalPrice * rate / 100m;
            var totalPayment = request.FinalPrice + feeAmount;
        ...
        public async Task<bool> ConfirmContractAsync(int contractId, int userId, string userRole)
        {
            ...
            if (contract.Status != "DRAFT")
                throw new InvalidOperationException("Contract is no longer editable.");
            ...
            if (userRole == "CLIENT")
            {
                ...
                if (clientProfile == null)
                    throw new UnauthorizedAccessException("Only the contract client can confirm this contract.");
            ...
            else if (userRole == "EXPERT")
            {
                ...
                if (expertProfile == null)
                    throw new UnauthorizedAccessException("Only the contract expert can confirm this contract.");
        ...
        private static void ValidateCreateRequest(CreateContractRequest request)
        {
            ...
            if (request.FinalTimelineDays <= 0)
                throw new InvalidOperationException("Timeline must be greater than 0.");
