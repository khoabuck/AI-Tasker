using System.Security.Claims;
using AITasker.Application.DTOs.Requests;
using AITasker.Application.Interfaces;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace AITasker.Api.Controllers;

[ApiController]
[Route("api/admin/platform-fee-policy")]
[Authorize(Roles = "ADMIN")]
public class AdminPlatformFeePoliciesController : ControllerBase
{
    private readonly IPlatformFeePolicyService _platformFeePolicyService;

    public AdminPlatformFeePoliciesController(
        IPlatformFeePolicyService platformFeePolicyService)
    {
        _platformFeePolicyService = platformFeePolicyService;
    }

    // GET /api/admin/platform-fee-policy
    [HttpGet]
    public async Task<IActionResult> GetActivePolicy()
    {
        var policy = await _platformFeePolicyService.GetActivePolicyAsync();

        return Ok(policy);
    }

    // PUT /api/admin/platform-fee-policy
    [HttpPut]
    public async Task<IActionResult> UpdateActivePolicy(
        [FromBody] UpdatePlatformFeePolicyRequest request)
    {
        try
        {
            var adminId = GetCurrentAdminId();

            var policy = await _platformFeePolicyService.UpdateActivePolicyAsync(
                adminId,
                request
            );

            return Ok(policy);
        }
        catch (InvalidOperationException ex)
        {
            return BadRequest(new
            {
                message = ex.Message
            });
        }
    }

    private int GetCurrentAdminId()
    {
        var userIdValue = User.FindFirstValue(ClaimTypes.NameIdentifier)
            ?? User.FindFirstValue("sub")
            ?? User.FindFirstValue("userId")
            ?? User.FindFirstValue("UserId");

        if (!int.TryParse(userIdValue, out var adminId))
        {
            throw new InvalidOperationException(
                "Cannot resolve current admin id from token."
            );
        }

        return adminId;
    }
}
