...
// Navbar dành riêng cho Client — có menu POST JOB, FIND JOBS, PROJECTS, FINANCE
// Khác Navbar guest — không có nút Login/Register, có avatar user
...
// ─── Component con: 1 nav item có dropdown ────────────
function NavItem({ label, to, dropdown, active }) {
  ...
  if (!dropdown) {
    return (
      <li>
        <Link
          to={to}
          className="flex items-center gap-2 h-full text-xs font-mono tracking-widest
  ...
  return (
    ...
    >
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute top-full left-0 w-52 bg-[#232A35] border border-white/10
                        rounded-b-xl shadow-2xl py-2 z-50">
          {dropdown.map((item) => (
            <Link
              key={item.label}
              to={item.to}
              className="flex items-center gap-3 px-4 py-2 text-sm text-white
                         hover:bg-white/10 hover:text-cyan-400 transition-colors"
            >
...
export default function ClientNavbar() {
  ...
  // Lấy 2 chữ cái đầu tên để hiện avatar
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#101319]/90 backdrop-blur-xl
                       border-b border-white/10 flex justify-between items-center
                       w-full px-4 md:px-12 py-4">
      ...
      {/* Nav items — chỉ hiện trên desktop */}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative group">
          <div className="w-8 h-8 rounded-full bg-blue-400 flex items-center justify-center
                          text-gray-900 font-bold text-sm cursor-pointer">
            {initials}
          </div>

          {/* Dropdown logout */}
          <div className="absolute right-0 top-full mt-2 w-40 bg-[#232A35] border border-white/10
            ...
            <button
              ...
              className="w-full text-left px-4 py-2 text-sm text-white
                         hover:bg-white/10 hover:text-red-400 transition-colors"
            >
              Logout
            </button>
          </div>
