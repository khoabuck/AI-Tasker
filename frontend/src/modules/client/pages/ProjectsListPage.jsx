...
export default function ProjectsListPage() {
    ...
    const fetchProjects = useCallback(async (signal) => {
        ...
        try {
            ...
            try {
                ...
                const reviews = Array.isArray(rawReviews) ? rawReviews : (rawReviews.items ?? rawReviews.data ?? []);

                const reviewedIds = new Set(
                    reviews.map((review) => review.projectId ?? review.project?.projectId).filter(Boolean)
                );
    ...
    const filteredProjects = allProjects.filter((p) => (p.status || '').toUpperCase() === activeStatus.toUpperCase());
            ...
            <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 24px' }}>
                ...
                <div style={{ marginBottom: 28 }}>
                    ...
                    <p style={{ color: '#8c90a0', fontSize: 14, margin: 0 }}>
                        Theo dõi tiến độ các project đang hợp tác.
                    </p>
                </div>
                            ...
                            <button
                                ...
                                <span
                                    style={{
                                        ...
                                        fontWeight: 700,
                                        fontFamily: 'JetBrains Mono, monospace',
                                        background: active ? tab.color + '20' : 'rgba(255,255,255,0.06)',
                                        color: active ? tab.color : '#8c90a0',
                                    }}>
                                    {count}
                                </span>
                ...
                {error && !loading && (
                    <div style={{ textAlign: 'center', padding: '60px 24px' }}>
                        <span
                            className="material-symbols-outlined"
                            style={{ fontSize: 48, color: '#f87171', display: 'block', marginBottom: 12 }}>
                            error_outline
                        </span>
                        <p style={{ color: '#f87171', fontSize: 15, marginBottom: 20 }}>{error}</p>
                        <button
                            onClick={() => fetchProjects(new AbortController().signal)}
                            style={{
                                padding: '10px 24px',
                ...
                {!loading && !error && filteredProjects.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '80px 0' }}>
                        <span
                            className="material-symbols-outlined"
                            style={{ fontSize: 72, display: 'block', marginBottom: 16, color: '#272a30' }}>
                            folder_off
                        </span>
                        <p style={{ color: '#8c90a0', fontSize: 16, marginBottom: 6 }}>
                            No projects are in this state. {STATUS_CONFIG[activeStatus]?.label}
                        </p>
                    </div>
                )}
