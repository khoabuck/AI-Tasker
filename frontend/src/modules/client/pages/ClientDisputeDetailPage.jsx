...
export default function ClientDisputeDetailPage() {
    ...
    const normalizedDisputeStatus = String(dispute?.status ?? '').toUpperCase();

    const isClosed = ['RESOLVED', 'REJECTED', 'CLOSED'].includes(normalizedDisputeStatus);
    ...
    const hasEvidenceAttachment = normalizedEvidenceFileUrl.length > 0 || evidenceImageUrls.length > 0;
    ...
    const fetchDispute = useCallback(
        async (signal, silent = false) => {
            ...
            try {
                ...
                const match = projectId ? list.find((d) => String(d.projectId) === String(projectId)) : list[0];
                ...
                if (err?.code === 'ERR_CANCELED' || err?.name === 'CanceledError' || signal?.aborted) {
                    return;
                }
    ...
    const handleUploadEvidenceImages = async (files) => {
        ...
        try {
            ...
            setUploadImageError(
                err?.message || err?.response?.data?.message || 'Image upload failed. Please try again.'
            );
        } finally {
    ...
    const handleAddEvidence = async () => {
        if (!canSubmitEvidence) {
            if (!hasEvidenceText) {
                setEvidenceError('Evidence description is required.');
            } else if (!hasEvidenceAttachment) {
                setEvidenceError('Provide a proof link or upload at least one image.');
            } else if (!isValidEvidenceFileUrl) {
                setEvidenceError('Evidence link must be a valid HTTP or HTTPS URL.');
            }
        ...
        try {
            ...
            setEvidenceError(err?.response?.data?.message || err?.response?.data?.title || 'Submit evidence failed.');
        } finally {
    ...
    const dStatus = DISPUTE_STATUS[normalizedDisputeStatus] || DISPUTE_STATUS.OPEN;
        ...
        return Array.from(
            new Set(
                [
                    ...
                    ...(Array.isArray(evidence?.images) ? evidence.images : []),
                ]
                    .map(getImageUrl)
                    .filter(Boolean)
            )
    ...
    const groupedEvidences = Object.values(
        evidences.reduce((groups, evidence, index) => {
            const evidenceCreatedAt = evidence?.createdAt ? new Date(evidence.createdAt) : null;

            const createdAtKey =
                evidenceCreatedAt && !Number.isNaN(evidenceCreatedAt.getTime())
            ...
            const groupKey =
                ...
                evidence?.batchId ||
                [evidence?.uploadedByUserId ?? '', evidence?.evidenceText?.trim() || '', createdAtKey].join('|');
            ...
            if (evidence?.evidenceId != null) {
                groups[groupKey].evidenceIds.push(evidence.evidenceId);
            }

            if (evidence?.fileUrl) {
                groups[groupKey].fileUrls.push(evidence.fileUrl);
            }

            if (Array.isArray(evidence?.fileUrls)) {
                groups[groupKey].fileUrls.push(...evidence.fileUrls);
            }

            groups[groupKey].imageUrls.push(...getEvidenceImages(evidence));

            groups[groupKey].fileUrls = Array.from(new Set(groups[groupKey].fileUrls.filter(Boolean)));

            groups[groupKey].imageUrls = Array.from(new Set(groups[groupKey].imageUrls.filter(Boolean)));
    ...
    return (
        <ClientLayout>
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 24px' }}>
                <button
                    onClick={() => navigate(projectId ? `/client/projects/${projectId}` : '/client/projects')}
                    style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 6,
                    ...
                    <div
                        ...
                        <span
                            style={{
                                ...
                                border: `1px solid ${dStatus.color}40`,
                                whiteSpace: 'nowrap',
                            }}>
                            {dStatus.label}
                        </span>
                    ...
                    <div style={{ ...cardStyle, marginBottom: 20 }}>
                        ...
                        {dispute.evidenceImageUrl && (
                            <img
                                ...
                                alt="Evidence"
                                onClick={() => setPreviewImageUrl(dispute.evidenceImageUrl)}
                                style={{
                                    width: 160,
                                    height: 160,
                                    objectFit: 'cover',
                            ...
                            {groupedEvidences.map((group, groupIndex) => (
                                <div
                                    ...
                                    {group.imageUrls.length > 0 && (
                                        <div
                                            style={{
                                                marginTop: group.evidenceText || group.fileUrls.length > 0 ? 12 : 0,
                                            }}>
                                            <span
                                                style={{
                                                    display: 'block',
                ...
                {!isClosed && (
                    <div style={cardStyle}>
                        ...
                        <div style={{ marginBottom: 12 }}>
                            ...
                            {uploadImageError && (
                                <p style={{ fontSize: 12, color: '#f87171', marginTop: 6 }}>{uploadImageError}</p>
                            )}
                                        ...
                                        <div key={url} style={{ position: 'relative' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    setEvidenceImageUrls((prev) => prev.filter((_, i) => i !== index));
                                                }}
                                                style={{
                                                    position: 'absolute',
                                                    top: 4,
                        ...
                        <button
                            ...
                            style={{
                                ...
                                color: evidenceSent ? '#002022' : canSubmitEvidence ? '#002022' : '#8c90a0',
                                ...
                                cursor: canSubmitEvidence ? 'pointer' : 'not-allowed',
