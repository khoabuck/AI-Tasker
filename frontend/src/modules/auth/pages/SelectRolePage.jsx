...
const ROLES = [
    {
        ...
        features: ['Post a job opening.', 'AI Matching Experts', 'Project Management & Payments'],
        color: '#00F0FF',
    },
    {
        key: 'EXPERT',
        ...
        features: ['Receive suitable job offers', 'AI-powered job recommendations', 'Build a professional portfolio'],
        color: '#adc6ff',
    },
];
...
export default function SelectRolePage() {
    ...
    const redirectExistingUser = async () => {
        ...
        const role = String(latestUser?.role || '')
            .trim()
            .toUpperCase();
        const status = String(latestUser?.status || '')
            .trim()
            .toUpperCase();
            ...
            const nextUser = normalizeUser(
                {
                    ...(user || {}),
                    ...(responseUser || {}),
                    role: responseUser?.role || responseUser?.Role || selected,
                    status: responseUser?.status || responseUser?.Status || 'PENDING_PROFILE',
                },
                selected
            );

            if (!nextUser) {
                throw new Error('Select role response does not contain valid user information.');
            }
            ...
            <nav
                ...
                <div
                    ...
                    <span
                        ...
                        <span style={{ color: '#00F0FF' }}>AI</span> <span style={{ color: '#e1e2eb' }}>Tasker</span>
                    </span>
                </div>
            </nav>
            ...
            <main
                ...
                <div style={{ maxWidth: 700, margin: '0 auto', padding: '0 16px' }}>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        if (isLockedOtherRole) {
                                            setError(
                                                'You already selected a role. Please continue with your current role.'
                                            );
                                            return;
                                        }
                                    ...
                                    style={{
                                        ...
                                        background: isSelected
                                            ? `rgba(${role.key === 'CLIENT' ? '0,240,255' : '173,198,255'},0.05)`
                                            : 'rgba(18,21,27,0.8)',
                                        ...
                                        transition: 'all 0.2s',
                                        boxShadow: isSelected ? `0 0 20px ${role.color}22` : 'none',
                                    }}>
                                    <div
                                        style={{
                                            width: 56,
                                            height: 56,
                                            borderRadius: 14,
                                            background: isSelected ? `${role.color}15` : 'rgba(255,255,255,0.05)',
                                            border: `1px solid ${isSelected ? role.color : 'rgba(255,255,255,0.1)'}`,
                                            display: 'flex',
                                            alignItems: 'center',
                    ...
                    <button
                        ...
                        style={{
                            width: '100%',
                            background: selected && !loading ? '#00F0FF' : 'rgba(255,255,255,0.05)',
                            ...
                            transition: 'all 0.2s',
                            boxShadow: selected && !loading ? '0 0 20px rgba(0,240,255,0.3)' : 'none',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            gap: 8,
...
function extractUserFromResponse(response) {
    ...
    return data?.data?.user || data?.data?.User || data?.user || data?.User || data?.data || null;
}
...
function normalizeUser(user, selectedRole) {
    ...
    return {
        ...
        authProvider: user?.authProvider || user?.AuthProvider || user?.provider || 'LOCAL',
        avatarUrl: user?.avatarUrl || user?.AvatarUrl || user?.photoUrl || user?.PhotoUrl || null,
    };
}
